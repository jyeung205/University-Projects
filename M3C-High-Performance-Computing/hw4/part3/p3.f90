!-------------------------------
!The advantage of using "smaller-squares" approach is that there is less boundary points so less communication.

!The disadvantage of using the "smaller-squares" approach is that it is harder to implement.
...
subroutine simulate_mpi(comm,numprocs,n,kmax,tol,g,S0,r0,t0,k_bc,C,deltaC)
    ...
    real(kind=8), allocatable, dimension(:,:) :: Clocalnew
    real(kind=8), allocatable, dimension(:,:) :: rinv2,fac,fac2,facp,facm,Cnew,Sdel2
    real(kind=8), allocatable,  dimension(:) :: deltaclocal

    allocate(deltaclocal(kmax))
    ...
    !-----------------
    allocate(rinv2(0:nlocal+1,0:n+1))
    allocate(fac(0:nlocal+1,0:n+1))
    allocate(facp(0:nlocal+1,0:n+1))
    allocate(facm(0:nlocal+1,0:n+1))
    allocate(fac2(0:nlocal+1,0:n+1))
    allocate(Sdel2(0:nlocal+1,0:n+1))

    !Update equation parameters------
    rinv2 = 1.d0/(r**2) !beta
    fac = 1.d0/(2.d0+2.d0*rinv2+del*del*g) !1/(del^2*k^2 + 2 + 2/r^2) !gamma
    facp = (1.d0 + 0.5d0*del/r)*fac !(1 + del/(2r))*fac
    facm = (1.d0 - 0.5d0*del/r)*fac !(1 - del/(2r))*fac
    fac2 = fac*rinv2

    allocate(clocalnew(0:nlocal+1,0:n+1))

    allocate(cu(0:nlocal+1))
    allocate(cl(0:nlocal+1))

    !set initial condition/boundary conditions
    Clocal = (sin(k_bc*t)**2)*(pi+1.d0-r)/pi
    Clocalnew = (sin(k_bc*t)**2)*(pi+1.d0-r)/pi
    ...
    do k = 1,kmax
      ...
      if (myid==0) then
        call MPI_SEND(cu,n+2,MPI_DOUBLE_PRECISION,myid+1,1,comm,ierr)
      end if

      if ((myid<numprocs-1) .AND.(MYID .NE. 0))  then
        call MPI_SEND(cu,n+2,MPI_DOUBLE_PRECISION,myid+1,1,comm,ierr)
        call MPI_SEND(cl,n+2,MPI_DOUBLE_PRECISION,myid-1,0,comm,ierr)
      end if
      ...
      if (myid==0) then
        call MPI_RECV(cl,n+2,MPI_DOUBLE_PRECISION,myid+1,0,comm,mpi_status_ignore,ierr)
        clocal(nlocal+1,:) = cl
      end if

      if ((myid<numprocs-1) .AND. (MYID .NE. 0)) then
        call MPI_RECV(cu,n+2,MPI_DOUBLE_PRECISION,myid-1,1,comm,mpi_status_ignore,ierr)
        call MPI_RECV(cl,n+2,MPI_DOUBLE_PRECISION,myid+1,0,comm,mpi_status_ignore,ierr)
        clocal(nlocal+1,:) = cl
        clocal(0,:) = cu
      end if
      ...
      !update whites
      do i1 = 1, nlocal
        if (mod(i1+istart-1,2) == 1) then
          do j1= 1, n, 2
            Clocalnew(i1,j1) = 0.5d0*(-Clocal(i1,j1) + 3.d0*(Sdel2(i1,j1) + Clocal(i1+1,j1)*facp(i1,j1) + &
                                Clocal(i1-1,j1)*facm(i1,j1) + (Clocal(i1,j1-1) + Clocal(i1,j1+1))*fac2(i1,j1) ))
          end do
        end if

        if (mod(i1+istart-1,2) == 0) then
          do j1= 2, n, 2
            Clocalnew(i1,j1) = 0.5d0*(-Clocal(i1,j1) + 3.d0*(Sdel2(i1,j1) + Clocal(i1+1,j1)*facp(i1,j1) + &
                                Clocal(i1-1,j1)*facm(i1,j1) + (Clocal(i1,j1-1) + Clocal(i1,j1+1))*fac2(i1,j1) ))
          end do
      ...
      do i1 = 1, nlocal
        if (mod(i1+istart-1,2) == 1) then
          do j1= 2, n, 2
            Clocalnew(i1,j1) = 0.5d0*(-Clocal(i1,j1) + 3.d0*(Sdel2(i1,j1) + Clocalnew(i1+1,j1)*facp(i1,j1) + &
                                Clocalnew(i1-1,j1)*facm(i1,j1) + (Clocalnew(i1,j1-1) + Clocalnew(i1,j1+1))*fac2(i1,j1) ))
          end do
        end if
      ...
      deltaclocal(k) = maxval(abs(Clocalnew(1:nlocal,1:n)-Clocal(1:nlocal,1:n)))
      call MPI_BARRIER(comm,ierr)
      call MPI_ALLREDUCE(deltaclocal(k),deltac(k),1,MPI_DOUBLE_PRECISION,MPI_MAX,comm,ierr)

      if (deltac(k)<tol) exit !check convergence criterion

      Clocal(1:nlocal,1:n) = Clocalnew(1:nlocal,1:n)
