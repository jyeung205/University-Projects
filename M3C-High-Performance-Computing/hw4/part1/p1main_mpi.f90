...
! To execute mpiexec -n 4 p1mpi.exe
...
program p1main_mpi
  ...
  !------------------
  integer ::  k      ! TAKE THIS OUT
  integer, dimension(:), allocatable :: seed     ! TAKE THIS OUT
  !-------------------
  ...
  !----------------------------------------
  if (myid==0) then                   ! TAKE THIS OUT
  call random_seed(size=k)         ! TAKE THIS OUT
  allocate(seed(1:k))      ! TAKE THIS OUT
  seed(:) = 10         ! TAKE THIS OUT
  call random_seed(put=seed)       ! TAKE THIS OUT
  end if
  !----------------------------------------
