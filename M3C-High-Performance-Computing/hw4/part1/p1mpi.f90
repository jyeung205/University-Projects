...
subroutine fgd_mpi(comm,fvec_guess,n,m,dlocal,d,alpha,fvec)
  ...
  real(kind=8), dimension(size(fvec_guess)) :: cgrad,cgradtotal
  integer :: myid,ierr
  ...
  do i1=1,i1max
    ...
    !-------
    call MPI_ALLREDUCE(cgrad, cgradtotal, size(fvec_guess),MPI_DOUBLE_PRECISION,MPI_SUM,COMM,ierr)
    call MPI_REDUCE(c, c_total, 1,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)

    !update fvec
    fvec = fvec - alpha*cgradtotal/dble(numprocs)
